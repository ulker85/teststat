...
//обработка нажатия кнопки "Редактировать отчеты"
	...
	foreach ($_POST as $k => $v) {
		if (substr($k, 0, 5) == 'edit_' && $v != 0) {
			...
			if (!sqlDateShort($date_f) && $date_f != 0) {
				$ERROR_MSG .= 'Введено некоректну дату: '.$date_f;
				continue;
			}
			
			if (!sqlDateShort($date_s) && $date_s != 0) {
				$ERROR_MSG .= 'Введено некоректну дату: '.$date_s;
				continue;
			}
...
//обработка нажатия кнопки "Редактировать бланки"
...
//-- подготовка массива со списком отчетности,
//-- которую сдает данное предприятие
//===========
		...
		if ( (($filtr_p == monthDate($row['num_srok']) || $filtr_p == 0) && $filtr_y == yearFullDate($row['num_srok'])) || !isset($row['num_srok'])) {
			if ($row['not_in_sukup'] != 1)
				$listItems[] = $row + array('form' => $row['name_f'].' ('.$row['period_f'].')',
													 ...
													 'type_f' => getListTypes($row['id_type_first'], '(оберіть спосіб)'),
													 'user_f' => getListUsers($row['id_user_first'], '(оберіть користув.)'),
													 'type_s' => getListTypes($row['id_type_second'], '(оберіть спосіб)'),
													 'user_s' => getListUsers($row['id_user_second'], '(оберіть користув.)'));
			else
...
//-- подготовка массива со списком взятых бланков
// Готовим условия отбора
$where = array();
...
$type = getListTypes(0, '(оберіть період)');
$user = getListUsers(0, '(оберіть користув.)');
